...
function hash(pw, len, chars) {
    
    var salt = passwd(pw, 64, chars);
    var runs = 0;
    
    for (var i = 0; i< salt.length; i++) {
        runs = runs + salt.charAt(i).charCodeAt();
    }
    
    runs = runs + parseInt(len);
    
    for (i = 0; i < runs; i++) {       
        var hasher = new jsSHA(salt + pw, "TEXT");
        pw = hasher.getHash("SHA-512", "HEX").toString();
        
    }
    return pw;
}

function passwd(str, len, chars) {

    var hasher = new jsSHA(str, "TEXT");
    var hash = hasher.getHash("SHA-512", "B64");
    
    var replacer = new jsSHA(hash, "TEXT");
    var replace = replacer.getHash("SHA-512", "B64");
    
    var passwd = hash.substring(0, len);
    
    if (chars) {
        
        var replaceables = ["\!", "\@", "\#", "\$", "\%", "\^", "\&", "\*", "\(", "\)", "\-", "\_", "\+", "\=", "\]", "\[", "\{", "\}", "\?", "\<", "\>"];
        
        var nums = hash.match(/\d/);
        var num = 0;
        var nums = parseInt(nums[0]);
        ...
        for (var i = 0; i < len/2; i++){
            ...
            passwd = passwd.replace(replace.charAt(i), replaceables[num]);
        }
    }
    return passwd;
}
...
self.addEventListener('message', function(e) {
    var pwd = passwd(hash(e.data.phrase, e.data.len, e.data.chars), e.data.len, e.data.chars);
    self.postMessage( {"pwd":pwd} );
}, false);