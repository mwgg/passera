...
var mwggPasseraHash = {
	hash: function(pw, len, chars) {
	   
	   this.pw = pw;
	   this.len = len;
	   this.chars = chars;
	   
	   this.salt = mwggPasseraHash.passwd(this.pw, 64, this.chars);
	   this.runs = 0;
	   
	   for (this.i = 0; this.i< this.salt.length; this.i++) {
	       this.runs = this.runs + this.salt.charAt(this.i).charCodeAt();
	   }
	   
	   this.runs = this.runs + parseInt(len);
	   
	   for (this.i = 0; this.i < this.runs; this.i++) {       
	       this.hasher = new jsSHA(this.salt + this.pw, "TEXT");
	       this.pw = this.hasher.getHash("SHA-512", "HEX").toString();
	       
	   }
	   return this.pw;
	},
	passwd: function(str, len, chars) {
		
	   this.str = str;
	   this.len = len;
	   this.chars = chars;
	   
	   this.hasher = new jsSHA(this.str, "TEXT");
	   this.hash = this.hasher.getHash("SHA-512", "B64");
	   
	   this.passwd = this.hash.substring(0, this.len);
	   
	   if (this.chars) {
		
		this.replacer = new jsSHA(this.hash, "TEXT");
		this.replace = this.replacer.getHash("SHA-512", "B64");
	       
		this.replaceables = ["\!", "\@", "\#", "\$", "\%", "\^", "\&", "\*", "\(", "\)", "\-", "\_", "\+", "\=", "\]", "\[", "\{", "\}", "\?", "\<", "\>"];
	       
		this.nums = this.hash.match(/\d/);
	       
	       this.num = 0;
	       this.nums = parseInt(this.nums[0]);
	       
	       for (this.i = 0; this.i < this.len/2; this.i++){
		   
		   this.num += this.nums;
		   if (this.num > 20) {
		       this.num -= 20;
		   }
		   this.passwd = this.passwd.replace(this.replace.charAt(this.i), this.replaceables[this.num]);
	       }
	   }
	   return this.passwd;
	}
...
self.onmessage = function (e) {
	mwggPasseraHash.pwd = mwggPasseraHash.passwd(mwggPasseraHash.hash(e.data.phrase, e.data.len, e.data.chars), e.data.len, e.data.chars);
	self.postMessage( {"pwd":mwggPasseraHash.pwd} );
};